'use strict';
angular.module('app.ui.chartControllers', [])

  .controller("n3ChartCtrl", function ($scope, $http) {
    $http.get('scripts/data/chartData1.json').success(function (data) {
      $scope.data1 = data;
    });
    $http.get('scripts/data/chartData2.json').success(function (data) {
      $scope.data2 = data;
    });
    $http.get('scripts/data/chartData3.json').success(function (data) {
      $scope.data3 = data;
    });
    $http.get('scripts/data/chartData4.json').success(function (data) {
      $scope.data4 = data;
    });
    $http.get('scripts/data/chartData5.json').success(function (data) {
      $scope.data5 = data;
    });
    $scope.data0 = [
      {
    ...
    $scope.xAxisTickFormatFunctionMonth = function () {
      return function (d) {
        return d3.time.format('%b')(new Date(d));
      }
    };
    $scope.xAxisTickFormatFunctionDay = function () {
      return function (d) {
  ...
  .controller("gaugeCtrl", function ($scope) {
    ...
    $scope.gaugeGreen = {
      ...
      opacity: 0.75,
      value: 35,
      textColor: '#4b5668', // text color
      arcColor: '#0fe57f',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: false
    };
    $scope.gaugeBlue = {
      name: 'February data',
      ...
      value: 48,
      textColor: '#4b5668', // text color
      arcColor: '#00b3f9',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugeRed = {
      ...
      value: 65,
      textColor: '#4b5668', // text color
      arcColor: '#fe3e5a',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugePurple = {
      ...
      value: 52,
      textColor: '#4b5668', // text color
      arcColor: '#996afe',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugeLightBlue = {
      name: 'May data',
      opacity: 0.75,
      value: 76,
      textColor: '#4b5668', // text color
      arcColor: '#00d2ed',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugeYellow = {
      ...
      value: 82,
      textColor: '#4b5668', // text color
      arcColor: '#feda02',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugeOrange = {
      ...
      value: 66,
      textColor: '#4b5668', // text color
      arcColor: '#fe7829',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
    $scope.gaugeInverse = {
      ...
      value: 47,
      textColor: '#4b5668', // text color
      arcColor: '#4b5668',  // animated arc color
      bgArcColor: '#efefef',   // round background under arc
      text: ' '
    };
  ...
  .controller("DoughnutCtrl", function ($scope) {
    $scope.labels = ["Download Sales", "In-Store Sales", "Mail-Order Sales", "User Visited",
      "Error Times", "Progress Times", "Session Times"];
    $scope.data = [300, 380, 150, 200, 250, 180, 270, 230];
  })
